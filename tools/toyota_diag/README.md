...
`monitor` is the human-facing Data List view: broad signal-name terms expand to matching DIDs, signals sharing a DID are coalesced, interactive terminals redraw a compact value table, and `--changed` suppresses unchanged rows. The default universal bundle selects the current-P5 family from Toyota's literal `GetSupportP5_DT.dll` binding, not from a locally maintained category allowlist or a generation-bit guess. The current shipped P5-bound categories are generation-low5 `0x14/0x15`; D1/D2 are resolved per category as `10 01` → `10 03`, while DD is also category-local (`22 F1 86`, `22 D1 00`, or a `10 03` extended-session refresh depending on the Toyota category). Cleanup deterministically restores D1. The older Camry v6 JSON remains a compatibility/evidence fixture and retains the broader raw SendProc `0x14/0x15/0x16` automatic-session gate; that gate is not used as the universal P5-family classifier. `--jsonl` emits one structured sample group per line and `--csv` emits one row per decoded signal sample.
...
`vehicle detect` obtains the VIN once through openpilot's standard read-only VIN query and evaluates Toyota's recovered current-Phase5 `CDbVinVehicleDecisionTable` wildcard rows. It no longer loops over maintained profiles using an F181 discriminator. `vehicle mounted` starts from the selected Toyota vehicle's own install sets, resolves each logical category through class-`0x10D` `CDbProtInfoTable`, and probes only the current-P5 candidates identified by Toyota's `GetSupportP5_DT.dll` binding. Phase `0x12` categories use their ordinary request IDs (for example Camry FRC `0x792`, Combination Meter `0x7C0`); phase `0x22` categories use upstream openpilot's existing extended-address ISO-TP shape (for example Camry TPM `0x750/0x2A`, Brake Booster `0x750/0x29`, Central Gateway `0x750/0x5F`). Unsupported-generation install rows remain visible as Toyota install metadata without pretending the P5 support query applies to them. Class-`0x10D +0x14` is preserved as Toyota's legislated physical request ID (`0x7E0..0x7E7`); the standard 11-bit OBD response used by the CLI is derived explicitly as request+8 (`0x7E8..0x7EF`). Timeouts remain observations, not absence claims. `did support CATEGORY [DID...]` uses the same Toyota route and the current `GetSupportP5_DT` DID path: root `22 01 01`, MSB-first `xx00` group bitmap, then selected `xx00` group bitmaps for `xx01..xxFF`; omit DIDs to enumerate the full advertised set.
