...
`tools/toyota` is the single Comma-side entry point for Toyota diagnostics recovered from Techstream/GTS+. The bundled `camry-2026-f33` registry is a generated, derived artifact: it contains Toyota's current vehicle/VIN resolver, install sets and 34 logical mount candidates, live P5 capability metadata, ECU names/validated direct addresses, GTS DID/DTC catalogs, static Active-Test plans, the exact F33 mutation identity guard, and the live-validated DTC-clear route. The CLI also bundles clean generated metadata for the current GTS+ TSS3 Operation/Image FFD protocols and PCS Data Viewer recorder schema. It does **not** contain Toyota DLL/DDB/EXE binaries.
...
`monitor` is the human-facing Data List view: broad signal-name terms expand to matching DIDs, signals sharing a DID are coalesced, interactive terminals redraw a compact value table, and `--changed` suppresses unchanged rows. Registry v5 uses the recovered current-P5 `DiagnosticSession` lifecycle according to Toyota's actual host gate: class-`0x110` category generation-low5 `0x14/0x15/0x16`. For eligible categories it inspects/polls F186, performs D1 `10 01` → D2 `10 03` when needed, periodically polls `22 F1 86`, and deterministically restores D1. The former local `[397,435,498]` proof subset is not a runtime gate. `--jsonl` emits one structured sample group per line and `--csv` emits one row per decoded signal sample.
...
`scan` produces a read-only vehicle inventory with responding endpoints, F181/F18C/0105 identity reads where supported, DTC status, active-fault summaries, transport state, and profile identity. It also carries all 34 Toyota logical mount candidates and annotates which candidates' independently validated direct endpoints responded to the existing DTC sweep; it does not collapse gateway-shared logical categories onto a single CAN ID.

`vehicle detect` now obtains the VIN once through openpilot's standard read-only VIN query and evaluates Toyota's recovered `CDbVinVehicleDecisionTable` wildcard rows. It no longer loops over profiles by reading each profile's F181 mutation guard. `vehicle mounted` shows the four Camry install sets and all 34 Toyota logical candidates, probing only the eight categories with independently validated direct Panda endpoints; `not_directly_routed` and `no_response` are deliberately observations rather than absence claims. `did support ECU DID...` implements the current `GetSupportP5_DT` DID path: root `22 01 01`, MSB-first `xx00` group bitmap, then the selected `xx00` group bitmap for `xx01..xxFF`.
...
Registry v5 keeps **static geometry** separate from **runtime authorization** while removing the old independent-tooling lifecycle policy. Its 428 Active-Test candidates contain 41 rows whose fixed request geometry is complete, 361 plan-only rows, and 26 unresolved rows. `executor.py` still rejects malformed/placeholder identifiers (for example a recovered `0xFFFF` RID) and requires complete wire geometry. Extended-session lifecycle eligibility now follows Toyota's recovered class-`0x110` generation-low5 gate `0x14/0x15/0x16`, so **38** geometry-complete rows are runtime-executable and **3** remain blocked by geometry (`RID=0xFFFF` placeholders); Engine and HV Battery are no longer rejected merely because they were outside our former three-category proof subset. Fixed routine controls such as FRC `0xA429` LTA Steering Vibration remain runtime-executable, while all direct `0x2F` tests remain plan-only because their exact runtime payload length is not recovered. The registry also carries raw Toyota CommSet rows (for example CommSet 1 `receive_timeout=1020`, retry count 1); the runtime exposes those rows but deliberately does not reinterpret the raw timeout as seconds until Techstream's `CheckAndConvertRcvTimeOut` conversion is fully recovered.
...
Use `--registry FILE` or `--profile PROFILE_NAME` to load another supported derived registry when additional vehicles are added. The loader accepts v1-v5 for backward compatibility. Engineering-value decoding requires explicit decoder metadata; topology/observed identities require the corresponding v3+ fields; execution/session behavior requires explicit v4 lifecycle and operation metadata; Toyota VIN/install-set/mount/P5-support resolution requires v5 `vehicle_resolution`. Nothing is inferred for older registries.
