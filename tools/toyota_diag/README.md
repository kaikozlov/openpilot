...
`vehicle detect` obtains the VIN once through openpilot's standard read-only VIN query and executes the recovered Toyota resolver stage appropriate to the VIN-source category. Phase5/Phase6 type-59 `CDbVinVehicleDecisionTable` hits are final at this stage. Phase3/Phase4 hits are explicitly non-final because Toyota next runs the Spe-master probe program and type-41 `CDbVehicleDecisionTable`; the CLI will not silently promote such a candidate to a selected vehicle until that live stage is materialized. VIN10 itself rejects generation-low5 5..19, but Toyota's master separately binds legacy `SelectCarType.dll`/`SelectCarTypeVin.dll`; those binaries are absent from the current GTS+ corpus, so that path is reported as unresolved rather than unsupported. `vehicle mounted` starts from the selected Toyota vehicle's own install sets and keeps every logical category. Class-`0x10D` supplies the category+phase route. P5/P6 categories with recovered live support roots are response-probed; other routed categories remain `probe_unavailable` rather than being removed or called an unsupported generation. Phase `0x12` categories use ordinary request IDs (for example Camry FRC `0x792`, Combination Meter `0x7C0`); phase `0x22` categories use upstream openpilot's existing extended-address ISO-TP shape (for example Camry TPM `0x750/0x2A`, Brake Booster `0x750/0x29`, Central Gateway `0x750/0x5F`). Class-`0x10D +0x14` is interpreted as a standard legislated physical request only when it lies in `0x7E0..0x7E7`; other values are preserved as route metadata rather than rejected. Timeouts remain observations, not absence claims. `did support CATEGORY [DID...]` dispatches through Toyota's selected support family; exact enumeration is currently implemented for P5 (`22 01 01` root, then MSB-first `xx00` group bitmaps). P6's recovered `22 A1 00` root is used as a live mounted-response witness, but full P6 DID enumeration remains an explicit implementation boundary until its hierarchy is reproduced exactly.
