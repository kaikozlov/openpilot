...
Live commands have two transport modes. If `pandad` is stopped, the CLI takes direct Panda ownership using Panda's ordinary ELM327 diagnostic safety mode. If `pandad` is already running, the CLI reuses openpilot's `can`/`sendcan` ISO-TP path when the single live Panda is already in ELM327 safety; the ELM327 safety parameter and `controlsAllowed` state are not additional CLI admission criteria. It never changes a running Panda's safety mode. If manager has transitioned Panda to an onroad safety model, the managed diagnostic path stops transmitting and directs you to use direct Panda access instead. `./tools/toyota transport status` checks this transport state without transmitting anything.
...
./tools/toyota vehicle mounted
./tools/toyota did support frc 0x1601 0x1914
./tools/toyota did support 452                  # Toyota category; uses 0x750/0x2A and enumerates advertised DIDs
./tools/toyota uds raw eps 0x22 F181
./tools/toyota uds raw 0x763 0x22 1033       # read-only unregistered endpoint
```

`uds raw` accepts an explicit 11-bit request address, including endpoints that are not part of the bundled vehicle profile. This is useful for recovered Toyota utility endpoints such as the MACKey-registration master `0x763`. Use `--sub-address` for an ISO-TP address-extension byte such as Toyota's logical `0x750` routes. Read-only services transmit directly; a service classified as mutating requires the explicit `--force` acknowledgement, with no separate registry-address or F181 admission check.
...
`monitor` is the human-facing Data List view: broad signal-name terms expand to matching DIDs, signals sharing a DID are coalesced, interactive terminals redraw a compact value table, and `--changed` suppresses unchanged rows. The current registry uses the recovered current-P5 `DiagnosticSession` lifecycle according to Toyota's actual host dispatch: class-`0x110` category generation-low5 `0x14/0x15/0x16`. For those generations it inspects/polls F186, performs D1 `10 01` → D2 `10 03` when needed, periodically polls `22 F1 86`, and deterministically restores D1. The former local `[397,435,498]` proof subset is not a runtime gate. `--jsonl` emits one structured sample group per line and `--csv` emits one row per decoded signal sample.
...
`vehicle detect` obtains the VIN once through openpilot's standard read-only VIN query and evaluates Toyota's recovered `CDbVinVehicleDecisionTable` wildcard rows. It no longer loops over profiles using an F181 discriminator. `vehicle mounted` shows the four Camry install sets and all 34 Toyota logical candidates, resolves every route from Toyota's class-`0x10D` `CDbProtInfoTable`, and queries all 33 current-P5 candidates through those routes. Phase `0x12` categories use their ordinary request IDs (for example FRC `0x792`, Combination Meter `0x7C0`); phase `0x22` categories use upstream openpilot's existing extended-address ISO-TP shape (for example TPM `0x750/0x2A`, Brake Booster `0x750/0x29`, Central Gateway `0x750/0x5F`). The one generation-4 Camry candidate is retained as Toyota install metadata without pretending the P5 support query applies to it. Timeouts remain observations, not absence claims. `did support CATEGORY [DID...]` uses the same Toyota route and the current `GetSupportP5_DT` DID path: root `22 01 01`, MSB-first `xx00` group bitmap, then selected `xx00` group bitmaps for `xx01..xxFF`; omit DIDs to enumerate the full advertised set.
...
Registry v6 keeps **complete request geometry** separate from incomplete plans while removing the old independent-tooling lifecycle policy. Its 428 Active-Test candidates contain 41 rows whose fixed request geometry is complete, 361 plan-only rows, and 26 unresolved rows. `executor.py` still rejects malformed/placeholder identifiers (for example a recovered `0xFFFF` RID) and requires all bytes needed to construct the request. Extended-session lifecycle selection follows Toyota's recovered class-`0x110` generation-low5 dispatch `0x14/0x15/0x16`, so **38** geometry-complete rows are runtime-executable and **3** remain blocked by geometry (`RID=0xFFFF` placeholders); Engine and HV Battery are no longer rejected merely because they were outside our former three-category proof subset. Fixed routine controls such as FRC `0xA429` LTA Steering Vibration remain runtime-executable, while all direct `0x2F` tests remain plan-only because their exact runtime payload length is not recovered. The registry also carries raw Toyota CommSet rows (for example CommSet 1 `receive_timeout=1020`, retry count 1); the runtime exposes those rows but deliberately does not reinterpret the raw timeout as seconds until Techstream's `CheckAndConvertRcvTimeOut` conversion is fully recovered.
