...
from tsk.lib.dump_dataflash import (
  ...
  DUMP_TOTAL,
  PAYLOAD_SHA256 as DATAFLASH_PAYLOAD_SHA256,
  dump as dump_dataflash,
  dump_path,
  partial_coverage_path,
  partial_dump_path,
)
...
from tsk.lib.ephemeral_runtime import (
  EphemeralRuntimeError,
  import_runtime_package_json,
...
from tsk.lib.target_profile import (
  ...
  persist_target_profile,
  public_target_profile_status,
...
PING_REPORT = "!!!! Unexpected error. Preserve the raw logs and export the " + "evidence bundle before continuing."
...
class PandaJob:
  """Own one background operation's state and shared-panda lifecycle."""
  ...
  def __init__(
    ...
    *,
    ...
    thread_name: str,
    start_error: str,
    start_updates: Callable[..., dict[str, Any]] | None = None,
    failure_updates: dict[str, Any] | None = None,
    ...
    self._start_updates = start_updates
    self._failure_updates = failure_updates or {}
  ...
  def update_present(self, **updates: Any) -> None:
    self.update(**{key: value for key, value in updates.items() if value is not None})
  ...
  def start(self, *args: Any, **kwargs: Any) -> bool:
    if not panda_lock.acquire(blocking=False):
      return False
    try:
      updates = self._start_updates(*args, **kwargs) if self._start_updates else {}
      with self._lock:
        self._replace_state(status="running", **updates)
      threading.Thread(
      ...
      self.update(status="failed", message=self._start_error)
      panda_lock.release()
      return False
    return True
  ...
  def _replace_state(self, **updates: Any) -> None:
    self._state.clear()
    self._state.update(deepcopy(self._initial_state))
    self._state.update(deepcopy(updates))
  ...
  def _execute(self, *args: Any, **kwargs: Any) -> None:
    try:
      ...
      updates = {"status": "failed", "message": str(e), **deepcopy(self._failure_updates)}
      if self._include_traceback:
        updates["traceback"] = traceback.format_exc()
      self.update(**updates)
    finally:
      try:
...
def get_ipv4_addresses() -> list[str]:
  ...
  try:
    for info in socket.getaddrinfo(socket.gethostname(), None, socket.AF_INET):
      append_address(addresses, str(info[4][0]))
  except OSError:
    pass
...
# Initial snapshots are immutable templates. PandaJob owns each operation's live
# state, lock, reset, thread startup, dry-run fallback, and panda cleanup.
_CAN_INITIAL_STATE = {
  "ready": False,
...
# DataFlash dump state. ready == (status == "complete") keeps UI gating stable.
_DATAFLASH_INITIAL_STATE = {
  "ready": False,
...
# Read-only CAN sniffer state; there is no persisted artifact to rehydrate.
_SNIFF_INITIAL_STATE = {
...
# Instrumented DataFlash diagnostic state. Counted SEND_KEY and payload writes
# retain their separate arming/evidence boundaries.
_DIAG_INITIAL_STATE = {
...
# Firmware-informed programming handoff probe state.
_PROBE_INITIAL_STATE = {
...
# ReadMemoryByAddress (0x23) probe state.
_READMEM_INITIAL_STATE = {
...
# Bounded application XCP observer state.
_XCP_INITIAL_STATE = {
...
# Audited inert ephemeral scheduler canary state.
_EPHEMERAL_INITIAL_STATE = {
...
# EPS identity + observation-oriented service-surface map state.
_IDENT_INITIAL_STATE = {
...
# Level 0x03 seed isolation probe state.
_LEVEL3_INITIAL_STATE = {
...
# Application 0x03/0x04 comparison state.
_SENDKEY_INITIAL_STATE = {
...
# Pre-programming preamble probe state.
_PREAMBLE_INITIAL_STATE = {
...
# Exhaustive Not Ready to Drive UDS sweep state.
_SWEEP_INITIAL_STATE = {
...
# READY passive capture state. This operation never transmits diagnostics.
_READY_INITIAL_STATE = {
...
# READY active diff state. It only replays prior Not Ready to Drive requests.
_READY_DIFF_INITIAL_STATE = {
...
def _identity_snapshot() -> dict:
  snapshot = JOBS["identity"].snapshot()
  snapshot["identity"] = [dict(row) for row in snapshot.get("identity", [])]
  return snapshot


def _current_app_sw() -> str:
  return _identity_value(_identity_snapshot().get("identity", []), "app_sw_id")
...
def ram_exec_geometry_status(identity: list[dict], *, fixture_sha256: str = DATAFLASH_PAYLOAD_SHA256) -> dict:
  ...
  bootstrap = (
    public_bootstrap_status(app_sw, fixture_sha256=fixture_sha256)
    if app_sw
    else {"compatible": False, "fixture_evidenced": False, "f181": "", "message": "EPS F181 is not identified."}
  )
  if geometry is None:
    return {
      "ready": False,
  ...
  return {
    ...
    "message": (
      ...
      if bootstrap.get("fixture_evidenced")
      else "Exact F181 has evidenced boot geometry, but selected DataFlash ciphertext is not target-evidenced."
    ),
  }
...
def persist_verified_recovery(key: str, verification: dict, *, source: str) -> tuple[dict, dict]:
  ...
  profile = persist_target_profile(identity_snapshot, verification=verification, oracle_path=can_oracle_path())
...
def dashboard_payload() -> dict:
  ...
  legacy_ram_fixture_ready = bool(
    app_sw and fixture_is_evidenced(app_sw, RAM_DUMP_FIXTURE_SHA256) and any(normalize_f181(raw) == app_sw for raw in TSKExtractor.APPLICATION_VERSIONS)
  )
  ...
  if installed and operational_profile_ready:
    ...
    next_action = {
      "id": "capture_can",
      "title": "Discover the target SecOC surface",
      "description": (
    ...
    next_action = {
      "id": "ram_geometry",
      "title": "Establish authenticated RAM-exec geometry",
      "description": (
      ...
      "href": "",
      "label": "RAM-exec geometry required",
      "vehicle_state": "Research / bench",
      "tone": "warning",
      "action": "research",
    }
    ...
    next_action = {
      "id": "bootstrap_fixture",
      "title": "Establish the exact bootstrap payload",
      "description": (
      ...
      "href": "/ephemeral-runtime.html",
      "label": "Review runtime/bootstrap evidence",
      "vehicle_state": "Research / bench",
      "tone": "warning",
      "action": "research",
    }
    ...
    if legacy_ram_fixture_ready and not dataflash_fixture_ready:
      next_action = {
        "id": "dataflash",
        "title": "Recover key material with the known RAM extractor",
        "description": (
    ...
    next_action = {
      "id": "verify",
      "title": "Recover and verify the target key",
      "description": (
    ...
    next_action = {
      "id": "integration",
      "title": "Complete the target integration profile",
      "description": (
      ...
      "href": "/target-profile.html",
      "label": "Review target profile",
      "vehicle_state": "Any",
      "tone": "primary",
    }
  elif not code_ready:
    ...
    next_action = {
      "id": "implementation",
      "title": "Implement and audit the target in opendbc",
      "description": (
      ...
      "href": "/target-profile.html",
      "label": "Review implementation audit",
      "vehicle_state": "Any",
      "tone": "primary",
    }
    ...
    next_action = {
      "id": "stationary",
      "title": "Run stationary acceptance verification",
      ...
      "href": "/stationary-verify.html",
      "label": "Stationary verification",
      "vehicle_state": "Stationary / bench",
      "tone": "primary",
    }
  elif not installed:
    stage = "install"
    next_action = {
      "id": "install",
      "title": "Install the verified target key",
    ...
    next_action = {
      "id": "integration",
      "title": "Reconcile the existing SecOCKey",
      "description": (
        "A SecOCKey is present, but no complete evidence-bound target profile proves it is safe for this integration. " + "Continue profile verification."
      ),
      "href": "/target-profile.html",
      "label": "Review target profile",
      "vehicle_state": "Any",
  ...
  if exact_f33:
    ...
    steps = [
      {"id": "identify", "title": "Identify EPS", "state": step_state(identity_ready, "identify"), "detail": app_sw or "Read F181 and diagnostic route"},
      {
        "id": "capture_can",
        "title": "Discover SecOC surface",
        "state": step_state(can_ready, "capture_can"),
        "detail": (
          f"{can.get('sync_count', 0)} sync · {len(included_streams)} candidate stream(s)"
          if can_ready
          else f"{can.get('sync_count', 0)} sync · capture incomplete"
        ),
      },
      {
        "id": "programming",
        "title": "Confirm recovery route",
        "state": step_state(key_recovered or programming_ready, "programming"),
        "detail": (
          "Key already recovered"
          if key_recovered
          else "Known transfer path"
          if known_transfer
          else "Bootloader reappeared"
          if programming_status == "entered"
          else "Required if DataFlash recovery is needed"
        ),
      },
      {
        "id": "ram_geometry",
        "title": "Verify boot RAM-exec geometry",
        "state": step_state(key_recovered or ram_geometry_ready, "ram_geometry"),
        "detail": (
          "Key already recovered"
          if key_recovered
          else f"{ram_geometry['geometry']['load_addr']} / {ram_geometry['geometry']['size']} evidenced for F181"
          if ram_geometry_ready
          else "Authenticated RequestDownload/0x10F0/callback geometry not established"
        ),
      },
      {
        "id": "bootstrap_fixture",
        "title": "Verify exact bootstrap fixture",
        "state": step_state(key_recovered or dataflash_fixture_ready or legacy_ram_fixture_ready, "bootstrap_fixture"),
        "detail": (
          "Key already recovered"
          if key_recovered
          else "DataFlash fixture target-evidenced"
          if dataflash_fixture_ready
          else "RAM extractor fixture target-evidenced"
          if legacy_ram_fixture_ready
          else "Bootstrap family known; exact ciphertext still required"
        ),
      },
      {
        "id": "dataflash",
        "title": "Recover key material",
        "state": step_state(key_recovered or dataflash_ready, "dataflash"),
        "detail": (
          "Key material recovered"
          if key_recovered
          else "Complete DataFlash dump"
          if dataflash.get("ready")
          else "Usable partial dump"
          if dataflash.get("status") == "partial"
          else "Blocked on boot RAM-exec geometry"
          if not ram_geometry_ready
          else "Blocked on exact bootstrap fixture"
          if not (dataflash_fixture_ready or legacy_ram_fixture_ready)
          else "Waiting"
        ),
      },
      {
        "id": "verify",
        "title": "Cryptographically recover key",
        "state": step_state(key_recovered, "verify"),
        "detail": (f"Recovered key {recovered.get('key_sha256_prefix', '')}" if key_recovered else "No operational install at this step"),
      },
      {
        "id": "integration",
        "title": "Review target integration",
        "state": step_state(integration_ready, "integration"),
      ...
      {
        "id": "implementation",
        "title": "Audit opendbc implementation",
        "state": step_state(code_ready, "implementation"),
      ...
      },
      {
        ...
        "state": step_state(stationary_ready, "stationary"),
        "detail": "Profile-bound acceptance passed" if stationary_ready else "Not yet proven on target",
      },
      {
        "id": "install",
        "title": "Install operational key",
        "state": step_state(installed and operational_profile_ready, "install"),
        "detail": "Installed after all gates"
        if installed and operational_profile_ready
        else "Existing unverified install"
        if installed
        else "Blocked until all gates pass",
      },
    ]
...
def _run_dataflash_job(use_recovery_payload: bool = False) -> None:
  ...
  previous = JOBS["dataflash"].snapshot()
  JOBS["dataflash"].update(
    status=status,
    frames=result.get("frames", previous["frames"]),
    bytes=result.get("bytes", previous["bytes"]),
...
def clear_dataflash() -> bool:
  # Refuse while a dump is in flight so a completing dump cannot recreate files
  # after the clear operation returns.
  if not JOBS["dataflash"].reset_if_idle():
    return False
...
def rehydrate_dataflash_state() -> None:
  ...
  if size == DUMP_TOTAL:
    JOBS["dataflash"].update(
      ready=True, status="complete", frames=DUMP_TOTAL // 4, bytes=DUMP_TOTAL, total=DUMP_TOTAL, size=DUMP_TOTAL, message="Dump complete."
    )
    return
  ...
  JOBS["dataflash"].update(
    ...
    total=DUMP_TOTAL,
    size=partial_size,
    message=("Partial dump on disk with byte coverage map.\n" if coverage_known else "Legacy partial dump on disk without a coverage map.\n")
...
def _can_progress(seconds=None, sync=None, protected=None) -> None:
  JOBS["can"].update_present(seconds=seconds, sync_count=sync, protected_count=protected)
...
def _run_can_mock() -> None:
  ...
  JOBS["can"].update(
    ...
    seconds=60.0,
    sync_count=60,
    protected_count=3600,
    protected_by_id={"0x131": 1200, "0x183": 1200, "0x2e4": 1200},
    counts_by_bus={1: {"sync": 60, "protected": 3600}},
    legacy_lateral_observed=True,
    legacy_lateral_counts={"0x131": 1200, "0x2e4": 1200},
    legacy_longitudinal_observed=True,
    legacy_longitudinal_counts={"0x183": 1200},
    profile_discovery={
      "streams": [
      ...
      "unknown_structural_candidates": 0,
      "unknown_scan_streams": 0,
      "scan_included_samples": 3600,
    },
    elm327_param=1,
...
def _run_can_job() -> None:
  ...
  status = result.get("status", "failed")
  previous = JOBS["can"].snapshot()
  JOBS["can"].update(
    status=status,
    sync_count=result.get("sync", previous["sync_count"]),
    protected_count=result.get("protected", previous["protected_count"]),
  ...
  if status == "complete" and public_recovered_key_status().get("recovered"):
    try:
      refresh_target_profile_from_recovered(_identity_snapshot(), oracle_path=can_oracle_path())
    except Exception as profile_error:
      current = JOBS["can"].snapshot()
      JOBS["can"].update(
        message=(current.get("message", "") + f" Profile refresh failed: {profile_error}").strip(),
      )


def clear_can() -> bool:
  # Refuse while collection is in flight so the finishing job cannot recreate
  # the oracle after the clear operation returns.
  if not JOBS["can"].reset_if_idle():
    return False
  try:
...
def rehydrate_can_state() -> None:
  ...
  lateral_counts = {f"0x{addr:03x}": by_addr.get(addr, 0) for addr in sorted(CURRENT_OPENPILOT_LATERAL_PROTECTED_ADDRS)}
  longitudinal_counts = {f"0x{addr:03x}": by_addr.get(addr, 0) for addr in sorted(CURRENT_OPENPILOT_LONGITUDINAL_PROTECTED_ADDRS)}
  ...
  JOBS["can"].update(
    ...
    message=(
      f"Persisted target-profile evidence: {len(analysis['sync_samples'])} sync samples, "
...
def _sniff_progress(seconds=None, frames=None, buses=None) -> None:
  JOBS["sniff"].update_present(seconds=seconds, frames=frames, bus_count=buses)
...
def _diag_progress(steps=None, last=None) -> None:
  JOBS["dataflash_diag"].update_present(step_count=steps, last=last)
...
def _run_diag_mock(allow_cross_calibration_send_key: bool = False) -> None:
  ...
  with JOBS["dataflash_diag"].locked_state() as diag_state:
    message = (
      ...
      if not allow_cross_calibration_send_key
      else "Unknown F181: explicitly armed bootloader SEND_KEY accepted; RAM-exec geometry remains unknown, so no WDBI or payload operation was sent. (mock)"
    )
    steps = [
...
def _run_diag_job(allow_cross_calibration_send_key: bool = False) -> None:
  result = dump_diagnose(
    progress_cb=_diag_progress,
    allow_cross_calibration_send_key=allow_cross_calibration_send_key,
  )
  JOBS["dataflash_diag"].update(
    status=result.get("status", "failed"),
...
def _probe_progress(attempts=None, last=None) -> None:
  JOBS["programming"].update_present(attempt_count=attempts, last=last)
...
def _run_probe_mock() -> None:
  ...
  route = {"eps_bus": 1, "eps_rx_bus": 1, "eps_tx": "0x7a1", "eps_rx": "0x7a9", "elm327_param": 1, "semantic_path": "normal-harness"}
  JOBS["programming"].update(
    status="entered",
    attempt_count=2,
    ...
    attempts=[
      {"name": "route discovery", "ok": True, "detail": str(route), "programming": False},
      {
        ...
        "ok": True,
        "detail": "endpoint reappeared on preserved route after response timeout",
        "programming": True,
      },
    ],
    security={},
    ...
    functional_0x777={
      "before": {"address": "0x777", "observed": True, "rx": "0x7a9", "rx_bus": 1},
    ...
    programming_handoff={
      ...
      "health_before": {"bus": 1},
      "health_after_reappearance": {"bus": 1},
    },
...
def _readmem_progress(reads=None, last=None) -> None:
  JOBS["read_memory"].update_present(count=reads, last=last)
...
def _run_readmem_mock() -> None:
  ...
  names = (
    "dataflash base",
    "object-15 / historical key region",
    "boot payload-key residue",
    "d/q observation bytes",
    "application SecurityAccess root mirror",
    "legacy no-memory-id key region",
  )
  for i, name in enumerate(names, 1):
    time.sleep(0.12)
    _readmem_progress(reads=i, last=name)
  JOBS["read_memory"].update(
    status="read",
    count=6,
    last=names[-1],
    panda="1.7.0-mock",
    ...
    application_sa_recovery={
      ...
      "memory_id": 1,
      "size": 16,
      "expected_root": "893e08418c741ffa2a9c044bffa55813",
      "evidence": "KEYLESS-006 exact-F181 startup LocalRAM mirror",
      "attempted": True,
      "recovered_root": "893e08418c741ffa2a9c044bffa55813",
    ...
    reads=[
      {
        ...
        "address": "0xff200000",
        "size": 16,
        "sienna_8965b4512000_policy": "firmware-readable",
        "request_data": "1502ff20000010",
        "ok": True,
        "detail": "112233445566778899aabbccddeeff00",
      },
      {
        "name": "object-15 / historical key region",
        "session": "extended",
        "shape": "memory-id",
        "memory_id": 2,
        "address": "0xff206e14",
        "size": 16,
        "sienna_8965b4512000_policy": "firmware-excluded",
        "request_data": "1502ff206e1410",
        "ok": False,
        "detail": "NRC 0x31 request out of range",
      },
      {
        "name": "boot payload-key residue",
        "session": "extended",
        "shape": "memory-id",
        "memory_id": 1,
        "address": "0xfebf2d08",
        "size": 16,
        "sienna_8965b4512000_policy": "firmware-readable",
        "request_data": "1501febf2d0810",
        "ok": True,
        "detail": "00112233445566778899aabbccddeeff",
      },
      {
        "name": "d/q observation bytes",
        "session": "extended",
        "shape": "memory-id",
        "memory_id": 1,
        "address": "0xfebe6d28",
        "size": 4,
        "sienna_8965b4512000_policy": "firmware-readable",
        "request_data": "1501febe6d2804",
        "ok": True,
        "detail": "00000000",
      },
      {
        "name": "application SecurityAccess root mirror",
        "session": "extended",
        "shape": "memory-id",
        "memory_id": 1,
        "address": "0xfebf7be0",
        "size": 16,
        "sienna_8965b4512000_policy": "firmware-readable",
        "request_data": "1501febf7be010",
        "ok": True,
        "detail": "893e08418c741ffa2a9c044bffa55813",
      },
      {
        "name": "legacy no-memory-id key region",
        "session": "extended",
        "shape": "ordinary",
        "memory_id": None,
        "address": "0xff206e14",
        "size": 16,
        "sienna_8965b4512000_policy": "different-alfid",
        "request_data": "14ff206e1410",
        "ok": False,
        "detail": "NRC 0x13 incorrect message length or invalid format",
      },
    ],
    message=" ".join(
      (
...
def _run_readmem_job() -> None:
  ...
  JOBS["read_memory"].update(
    ...
    panda=result.get("panda", ""),
    eps_bus=result.get("eps_bus", -1),
    eps_rx_bus=result.get("eps_rx_bus", -1),
    eps_tx=result.get("eps_tx", ""),
    eps_rx=result.get("eps_rx", ""),
    f181=result.get("f181", ""),
    f181_hex=result.get("f181_hex", ""),
    reads=result.get("reads", []),
    count=len(result.get("reads", [])),
...
def _xcp_progress(step=None, last=None) -> None:
  JOBS["xcp_observer"].update_present(
...
def _run_xcp_mock(profile: str) -> None:
  ...
  for step, last in (
    ("connect", "CAN 0x7F7 CONNECT"),
    ("snapshot", "bounded F4 LocalRAM reads"),
    ("daq", f"configure {profile}"),
    ("capture", "capture 0x7F8 DAQ DTOs"),
  ):
    time.sleep(0.12)
    _xcp_progress(step=step, last=last)
  snapshot = [{"address": f"0x{address:08x}", "ok": True, "value": index} for index, address in enumerate(selected.addresses)]
  sample_values = [{"address": row["address"], "value": row["value"]} for row in snapshot[:7]]
  JOBS["xcp_observer"].update(
    status="observed",
    count=2,
    panda="1.7.0-mock",
    eps_bus=1,
    ...
    profile_finding_ids=list(selected.finding_ids),
    xcp_request_id="0x7f7",
    xcp_response_id="0x7f8",
    connect_response="ff00000000000000",
    snapshot=snapshot,
    frames=[
      {"pid": 0, "t_ms": 0.0, "raw": "0001020304050607", "values": sample_values},
      {"pid": 0, "t_ms": 2.0, "raw": "0002030405060708", "values": sample_values},
    ],
    profile_semantics_verified=True,
    profile_semantics="firmware-verified for exact 8965B4512000",
    volatile_daq_configuration=True,
    source_memory_writes_implemented=False,
    write_commands_implemented=False,
    ...
    wall_clock_rate_claimed=False,
    control_timing={
      "requests": [{"operation": "connect", "request_hex": "ff00000000000000", "rtt_seconds": 0.001}],
      "rtt_statistics": {
        "count": 1,
        "min_seconds": 0.001,
        "max_seconds": 0.001,
...
def _run_xcp_job(profile: str) -> None:
  ...
  JOBS["xcp_observer"].update(
    ...
    connect_response=result.get("connect_response", ""),
    snapshot=result.get("snapshot", []),
    frames=result.get("frames", []),
    count=len(result.get("frames", [])),
...
def _ephemeral_progress(step=None, message=None) -> None:
  JOBS["ephemeral_canary"].update_present(
    step=str(step) if step is not None else None,
...
def _run_ephemeral_mock(f181: str) -> None:
  ...
  result = {
    ...
    "reset_to_stock_verified": True,
    "bridge_execution_exposed": False,
    "mock": True,
  }
  JOBS["ephemeral_canary"].update(status="passed", result=result, message="Inert scheduler canary passed (mock).")
...
def _run_ephemeral_canary_job(f181: str) -> None:
  ...
  JOBS["ephemeral_canary"].update(
    status="passed",
    result=result,
    message=("Inert scheduler canary heartbeat and reset-to-stock proof passed. " + "The steering bridge remains unavailable in TSK."),
  )


def _ident_progress(items=None, last=None) -> None:
  JOBS["identity"].update_present(count=items, last=last)
...
def _run_ident_mock() -> None:
  ...
  JOBS["identity"].update(
    status="mapped",
    count=24,
    last="0x19 read DTC info",
    panda="1.7.0-mock",
    eps_bus=1,
    eps_rx_bus=1,
    eps_tx="0x7a1",
...
def _reset_progress(attempts=None, last=None) -> None:
  JOBS["reset"].update_present(count=attempts, last=last)
...
def _run_reset_job() -> None:
  ...
  JOBS["reset"].update(
    ...
    attempts=result.get("attempts", []),
    count=len(result.get("attempts", [])),
    session_after=result.get("session_after", ""),
    message=result.get("message", ""),
    **_route_metadata(result),
  )
...
def _level3_progress(tests=None, last=None) -> None:
  JOBS["level3"].update_present(count=tests, last=last)


def _run_level3_mock() -> None:
  # Laptop dry run: the reproduced shape — 0x03 answers from a clean extended session.
  for i, name in enumerate(("clean extended (0x03 first)", "default session (no extended)", "0x01 first, then 0x03", "programming poke, then 0x03"), 1):
    time.sleep(0.3)
    _level3_progress(tests=i, last=name)
  JOBS["level3"].update(
    status="reproduced",
    count=4,
    last="programming poke, then 0x03",
    panda="1.7.0-mock",
    eps_bus=1,
    seeds=["da2df2eff64d95f5426bf3af70bb49aa", "1c9a4f0b77e3d5218a6c4b0fe29d3a11", "77aa10c4be5518e2049f3c6d1b8e720d", "9be1035fac82d4761e0b5528cf94a6d3"],
    primer="",
    tests=[
      {
        "name": "clean extended (0x03 first)",
        "got_seed": True,
      ...
      {
        "name": "default session (no extended)",
        "got_seed": False,
        "seed": "",
        "steps": [
          {"step": "default", "detail": "accepted"},
      ...
      {
        "name": "programming poke, then 0x03",
        "got_seed": True,
...
def _sendkey_progress(step=None, last=None) -> None:
  JOBS["send_key"].update_present(last=last)
...
def _run_sendkey_mock(allow_cross_calibration: bool = False) -> None:
  ...
  time.sleep(0.2)
  with JOBS["send_key"].locked_state() as sendkey_state:
    if not allow_cross_calibration:
      sendkey_state.update(
        ...
        send_key="",
        post_unlock_reads=[],
        target_f181=".8965F1208000....",
        target_f181_hex="0138393635463132303830303000000000",
        cross_calibration=True,
        armed=False,
...
def _run_sendkey_job(allow_cross_calibration: bool = False) -> None:
  result = send_sienna_application_key(progress_cb=_sendkey_progress, allow_cross_calibration=allow_cross_calibration)
  JOBS["send_key"].update(
    status=result.get("status", "failed"),
    panda=result.get("panda", ""),
    eps_bus=result.get("eps_bus", -1),
...
def _preamble_progress(steps=None, last=None) -> None:
  JOBS["preamble"].update_present(count=steps, last=last)
...
def _run_preamble_mock() -> None:
  ...
  JOBS["preamble"].update(
    ...
    services=[
      ...
      {"name": "read DID 0x201 (did_201_key)", "ok": False, "detail": "NRC 0x31 requestOutOfRange"},
      {"name": "read DID 0x202 (did_202_iv)", "ok": False, "detail": "NRC 0x31 requestOutOfRange"},
      {"name": "read DID 0x203 (did_203_state)", "ok": False, "detail": "NRC 0x31 requestOutOfRange"},
      {"name": "routine results 0x10f0", "ok": False, "detail": "NRC 0x7f serviceNotSupportedInActiveSession"},
      {"name": "request download (RAM)", "ok": False, "detail": "NRC 0x7f serviceNotSupportedInActiveSession"},
    ...
    dtc={"before": ["c11234:08", "c15678:2f", "c1aa01:08"], "after": ["c11234:08", "c15678:2f", "c1aa01:08"], "new": []},
    reads=[
...
def _sweep_progress(steps=None, last=None, stage=None) -> None:
  JOBS["uds_sweep"].update_present(count=steps, last=last, stage=stage)
...
def _run_sweep_mock() -> None:
  ...
  for i, (stage, last) in enumerate(
    (
      ...
      ("subfunctions", "sub-functions of 0x10"),
      ("subfunctions", "sub-functions of 0x22"),
      ("dids", "DIDs identity"),
      ("addresses", "address sweep"),
      ("cross", "cross-ECU silent set"),
    ),
    1,
  ):
    time.sleep(0.25)
    _sweep_progress(steps=i * 64, last=last, stage=stage)
  JOBS["uds_sweep"].update(
    status="partial",
    count=812,
    last="sub-functions of 0x27",
    stage="subfunctions",
    panda="1.7.0-mock",
    eps_tx="0x7a1",
    eps_bus=1,
...
def _run_sweep_job() -> None:
  ...
  JOBS["uds_sweep"].update(
    ...
    answering=result.get("answering", []),
    silent=result.get("silent", []),
    responders=result.get("responders", []),
    responder_routes=result.get("responder_routes", []),
...
def _ready_progress(steps=None, last=None, stage=None) -> None:
  JOBS["ready_passive"].update_present(count=steps, last=last, stage=stage)


def _run_ready_mock() -> None:
  for i, last in enumerate(("capturing 30s / 90s", "capturing 90s / 90s", "passive capture analysed"), 1):
    time.sleep(0.3)
    _ready_progress(steps=i * 14000, last=last, stage="passive capture")
  JOBS["ready_passive"].update(
    status="captured",
    count=41208,
    last="passive capture analysed",
    stage="passive capture",
    mode="passive",
    run_id="ready-passive-mock",
    panda="1.7.0-mock",
    eps_bus=-1,
    frames=41208,
    tx_echoes_filtered=0,
    ...
    capture={
      ...
      "hypothesis_hits": [
        {"bus": 1, "addr": "0x2e4", "samples": 20, "annotation": "prior Sienna/Corolla protected-ID hypothesis"},
      ],
      "sync": [{"bus": 1, "addr": "0x00f", "samples": 88, "distinct": 88, "annotation": "prior Toyota sync hypothesis"}],
    },
    diff=[],
    responders=[],
    cross=[],
...
def _ready_diff_progress(steps=None, last=None, stage=None) -> None:
  JOBS["ready_active_diff"].update_present(count=steps, last=last, stage=stage)
...
def _run_ready_diff_mock() -> None:
  ...
  JOBS["ready_active_diff"].update(
    status="complete",
    count=3,
    last="service 0x85",
    stage="active diff",
    mode="active_diff",
    run_id="ready-diff-mock",
    panda="1.7.0-mock",
    eps_tx="0x7a1",
    eps_bus=1,
    eps_rx_bus=1,
    ...
    frames=0,
    path="/cache/tsk/uds-sweep/ready_diff.ndjson",
...
def _run_ready_diff_job() -> None:
  ...
  JOBS["ready_active_diff"].update(
    status=result.get("status", "failed"),
    mode=result.get("mode", "active_diff"),
    run_id=result.get("run_id", ""),
    panda=result.get("panda", ""),
    eps_tx=result.get("eps_tx", ""),
    eps_bus=result.get("eps_bus", -1),
...
JOBS: dict[str, PandaJob] = {
  "can": PandaJob(
    _CAN_INITIAL_STATE,
    run=_run_can_job,
    mock=_run_can_mock,
    thread_name="tsk_can_collect",
    start_error="Could not start the collection job.",
    failure_updates={"ready": False},
  ),
  "dataflash": PandaJob(
    _DATAFLASH_INITIAL_STATE,
    run=_run_dataflash_job,
    mock=lambda use_recovery_payload=False: _run_dataflash_mock(),
    thread_name="tsk_dataflash_dump",
    start_error="Could not start the dump job.",
    start_updates=lambda use_recovery_payload=False: {
      "payload_variant": "auto-reset-experimental" if use_recovery_payload else "standard",
    },
    failure_updates={"ready": False, "size": 0},
  ),
  "sniff": PandaJob(
    _SNIFF_INITIAL_STATE,
    run=_run_sniff_job,
    mock=_run_sniff_mock,
    thread_name="tsk_can_sniff",
    start_error="Could not start the sniff job.",
  ),
  "dataflash_diag": PandaJob(
    _DIAG_INITIAL_STATE,
    run=_run_diag_job,
    mock=_run_diag_mock,
    thread_name="tsk_dataflash_diag",
    start_error="Could not start the diagnostic job.",
    start_updates=lambda allow_cross_calibration_send_key=False: {
      "send_key_armed": allow_cross_calibration_send_key,
    },
    include_traceback=True,
  ),
  "programming": PandaJob(
    _PROBE_INITIAL_STATE,
    run=_run_probe_job,
    mock=_run_probe_mock,
    thread_name="tsk_prog_probe",
    start_error="Could not start the probe job.",
  ),
  "read_memory": PandaJob(
    _READMEM_INITIAL_STATE,
    run=_run_readmem_job,
    mock=_run_readmem_mock,
    thread_name="tsk_read_mem",
    start_error="Could not start the read-memory job.",
  ),
  "xcp_observer": PandaJob(
    _XCP_INITIAL_STATE,
    run=_run_xcp_job,
    mock=_run_xcp_mock,
    thread_name="tsk_xcp_observer",
    start_error="Could not start the XCP observer job.",
    start_updates=lambda profile: {
      "profile": profile,
      "profile_description": XCP_PROFILES[profile].description,
      "profile_finding_ids": list(XCP_PROFILES[profile].finding_ids),
      "cleanup_error": "",
      "daq_error": "",
    },
  ),
  "ephemeral_canary": PandaJob(
    _EPHEMERAL_INITIAL_STATE,
    run=_run_ephemeral_canary_job,
    mock=_run_ephemeral_mock,
    thread_name="tsk_ephemeral_canary",
    start_error="Could not start the inert canary job.",
    start_updates=lambda f181: {
      "f181": normalize_f181(f181),
      "package": public_runtime_status(normalize_f181(f181)),
    },
    failure_updates={"result": {}},
  ),
  "identity": PandaJob(
    _IDENT_INITIAL_STATE,
    run=_run_ident_job,
    mock=_run_ident_mock,
    thread_name="tsk_ident_map",
    start_error="Could not start the identity-map job.",
  ),
  "reset": PandaJob(
    _RESET_INITIAL_STATE,
    run=_run_reset_job,
    mock=_run_reset_mock,
    thread_name="tsk_reset_probe",
    start_error="Could not start the reset-probe job.",
  ),
  "level3": PandaJob(
    _LEVEL3_INITIAL_STATE,
    run=_run_level3_job,
    mock=_run_level3_mock,
    thread_name="tsk_level3_probe",
    start_error="Could not start the level-0x03 probe.",
  ),
  "send_key": PandaJob(
    _SENDKEY_INITIAL_STATE,
    run=_run_sendkey_job,
    mock=_run_sendkey_mock,
    thread_name="tsk_sendkey_probe",
    start_error="Could not start the send-key probe.",
    start_updates=lambda allow_cross_calibration=False: {"armed": allow_cross_calibration},
  ),
  "preamble": PandaJob(
    _PREAMBLE_INITIAL_STATE,
    run=_run_preamble_job,
    mock=_run_preamble_mock,
    thread_name="tsk_preamble_probe",
    start_error="Could not start the preamble probe.",
  ),
  "uds_sweep": PandaJob(
    _SWEEP_INITIAL_STATE,
    run=_run_sweep_job,
    mock=_run_sweep_mock,
    thread_name="tsk_uds_sweep",
    start_error="Could not start the UDS sweep.",
  ),
  "ready_passive": PandaJob(
    _READY_INITIAL_STATE,
    run=_run_ready_job,
    mock=_run_ready_mock,
    thread_name="tsk_ready_capture",
    start_error="Could not start the READY capture.",
    start_updates=lambda: {"stage": "passive capture"},
  ),
  "ready_active_diff": PandaJob(
    _READY_DIFF_INITIAL_STATE,
    run=_run_ready_diff_job,
    mock=_run_ready_diff_mock,
    thread_name="tsk_ready_diff",
    start_error="Could not start the READY active diff.",
    start_updates=lambda: {"stage": "active diff"},
  ),
}
...
class TSKWebHandler(BaseHTTPRequestHandler):
      ...
      self._send_json(
        {
          ...
          "recovered_key": recovered,
          "message": (
            f"Cryptographically verified key recovered (dry run):\n{format_key(DRY_RUN_FAKE_KEY)}\n\n"
  ...
  POST_ROUTES = {
    "/api/extract": "_post_extract",
    "/api/match": "_post_match",
    "/api/refresh-target-profile": "_post_refresh_target_profile",
    "/api/target-profile-manifest": "_post_target_profile_manifest",
    "/api/stationary-verify": "_post_stationary_verify",
    "/api/install-recovered-key": "_post_install_recovered_key",
    "/api/uninstall": "_post_uninstall",
    "/api/reboot": "_post_reboot",
    "/api/ephemeral-runtime-package": "_post_ephemeral_runtime_package",
    "/api/ephemeral-canary": "_post_ephemeral_canary",
    "/api/can-collect": "_post_can_collect",
    "/api/dataflash-dump": "_post_dataflash_dump",
    "/api/can-sniff": "_post_can_sniff",
    "/api/prog-probe": "_post_prog_probe",
    "/api/dataflash-diag": "_post_dataflash_diag",
    "/api/read-mem": "_post_read_mem",
    "/api/xcp-observer": "_post_xcp_observer",
    "/api/ident-map": "_post_ident_map",
    "/api/reset-probe": "_post_reset_probe",
    "/api/level3-probe": "_post_level3_probe",
    "/api/uds-sweep": "_post_uds_sweep",
    "/api/ready-capture": "_post_ready_capture",
    "/api/ready-diff": "_post_ready_diff",
    "/api/preamble-probe": "_post_preamble_probe",
    "/api/sendkey-probe": "_post_sendkey_probe",
    "/api/clear-cache": "_post_clear_cache",
  }

  def do_POST(self) -> None:
    path = urlparse(self.path).path
    try:
      record_operation(path, client=self.client_address[0], vehicle_state=expected_vehicle_state(path))
    except OSError:
      pass
        ...
        self._send_json(
          {
            ...
            "profile_discovery": {"streams": oracle_analysis.get("streams", []), "can_inventory": oracle_analysis.get("can_inventory", [])},
            "message": (
              "Collect target-profile CAN evidence before extraction. Current usable oracle: "
        ...
        self._send_json(
          {
            ...
            "message": (
              "RAM extraction produced a checksum-valid KEY_4 candidate, but it did not pass "
  ...
  def _post_match(self) -> None:
    ...
    try:
      ...
      if result["status"] == "found":
        ...
        detail = f"Found at {result['address']} — {result['matches']} matches " + f"(sync {result['sync']}, protected {result['protected']})."
        message = (
  ...
  def _post_refresh_target_profile(self) -> None:
    try:
      if not public_recovered_key_status().get("recovered"):
        self._send_json(
          {"ok": False, "status": "no_recovered_key", "message": "Recover a cryptographically verified key before refreshing the target profile."},
          status=HTTPStatus.CONFLICT,
        )
        return
      identity_snapshot = _identity_snapshot()
      ...
      self._send_json({"ok": False, "status": "error", "message": str(e), "traceback": traceback.format_exc()}, status=HTTPStatus.INTERNAL_SERVER_ERROR)
    return

  def _post_target_profile_manifest(self) -> None:
    try:
      ...
      self._send_json({"ok": False, "status": "invalid_manifest", "message": str(e)}, status=HTTPStatus.CONFLICT)
    except Exception as e:
      self._send_json({"ok": False, "status": "error", "message": str(e), "traceback": traceback.format_exc()}, status=HTTPStatus.INTERNAL_SERVER_ERROR)
    return

  def _post_stationary_verify(self) -> None:
    try:
      request = self._read_json_body()
      identity_snapshot = _identity_snapshot()
      ...
      self._send_json({"ok": result.get("status") == "passed", "result": result, "target_profile": profile}, status=status)
    except ValueError as e:
      self._send_json({"ok": False, "status": "invalid_stationary_evidence", "message": str(e)}, status=HTTPStatus.CONFLICT)
    except Exception as e:
      self._send_json({"ok": False, "status": "error", "message": str(e), "traceback": traceback.format_exc()}, status=HTTPStatus.INTERNAL_SERVER_ERROR)
    return

  def _post_install_recovered_key(self) -> None:
    try:
      recovered = public_recovered_key_status()
      profile = public_target_profile_status()
      readiness = profile.get("readiness", {})
        ...
        self._send_json(
          {
            ...
            "message": (
              "Recovered key is intentionally not installable yet. " + "Target integration and profile-bound stationary verification must pass first."
            ),
  ...
  def _post_ephemeral_runtime_package(self) -> None:
    try:
      request = self._read_json_body()
      app_sw = _current_app_sw()
      if not app_sw:
        self._send_json(
          {
            "ok": False,
      ...
      self._send_json({"ok": False, "status": "invalid_runtime_package", "message": str(e)}, status=HTTPStatus.CONFLICT)
    except Exception as e:
      self._send_json({"ok": False, "status": "error", "message": str(e), "traceback": traceback.format_exc()}, status=HTTPStatus.INTERNAL_SERVER_ERROR)
    return

  def _post_ephemeral_canary(self) -> None:
    request = self._read_json_body()
    if not bool(request.get("bench_isolated", False)):
      self._send_json(
        {
          "ok": False,
          "status": "bench_ack_required",
    ...
    if not runtime.get("live_canary_ready"):
      ...
      return
    if not JOBS["ephemeral_canary"].start(app_sw):
      self._send_json(
        {
          "ok": False,
          "status": "running",
  ...
  def _post_can_collect(self) -> None:
    if not JOBS["can"].start():
      self._send_json(
        {
          "ok": False,
          "status": "running",
  ...
  def _post_dataflash_dump(self) -> None:
    request = self._read_json_body()
    ...
    geometry_gate = ram_exec_geometry_status(
      _identity_snapshot().get("identity", []),
      fixture_sha256=selected_fixture_sha,
    )
      ...
      self._send_json(
        {
          ...
          "ram_exec_geometry": geometry_gate,
          "message": (
            geometry_gate["message"] + " The selected DataFlash payload was not sent; exact fixture acceptance is independent from shared boot geometry."
          ),
        },
        status=HTTPStatus.CONFLICT,
      )
      return
    if not JOBS["dataflash"].start(use_recovery_payload=use_recovery_payload):
      self._send_json(
        {
          "ok": False,
          "status": "running",
      ...
      return
    self._send_json({"ok": True, "status": "running", "payload_variant": "auto-reset-experimental" if use_recovery_payload else "standard"})
    return

  def _post_can_sniff(self) -> None:
    if not JOBS["sniff"].start():
      self._send_json(
        {
          "ok": False,
          "status": "running",
  ...
  def _post_prog_probe(self) -> None:
    if not JOBS["programming"].start():
      self._send_json(
        {
          "ok": False,
          "status": "running",
  ...
  def _post_dataflash_diag(self) -> None:
    request = self._read_json_body()
    allow_cross_calibration_send_key = bool(request.get("allow_cross_calibration_send_key", False))
    if not JOBS["dataflash_diag"].start(allow_cross_calibration_send_key=allow_cross_calibration_send_key):
      self._send_json(
        {
          "ok": False,
          "status": "running",
  ...
  def _post_read_mem(self) -> None:
    if not JOBS["read_memory"].start():
      self._send_json(
        {
          "ok": False,
          "status": "running",
  ...
  def _post_xcp_observer(self) -> None:
    request = self._read_json_body()
    ...
    if profile not in XCP_PROFILES:
      ...
      return
    if not JOBS["xcp_observer"].start(profile):
      self._send_json(
        {
          "ok": False,
          "status": "running",
  ...
  def _post_ident_map(self) -> None:
    if not JOBS["identity"].start():
      self._send_json(
        {
          "ok": False,
          "status": "running",
  ...
  def _post_reset_probe(self) -> None:
    if not JOBS["reset"].start():
      self._send_json(
        {
          "ok": False,
          "status": "running",
  ...
  def _post_level3_probe(self) -> None:
    if not JOBS["level3"].start():
      self._send_json(
        {
          "ok": False,
          "status": "running",
  ...
  def _post_uds_sweep(self) -> None:
    if not JOBS["uds_sweep"].start():
      self._send_json(
        {
          "ok": False,
          "status": "running",
  ...
  def _post_ready_capture(self) -> None:
    if not JOBS["ready_passive"].start():
      self._send_json(
        {
          "ok": False,
          "status": "running",
  ...
  def _post_ready_diff(self) -> None:
    if not JOBS["ready_active_diff"].start():
      self._send_json(
        {
          "ok": False,
          "status": "running",
  ...
  def _post_preamble_probe(self) -> None:
    if not JOBS["preamble"].start():
      self._send_json(
        {
          "ok": False,
          "status": "running",
  ...
  def _post_sendkey_probe(self) -> None:
    request = self._read_json_body()
    allow_cross_calibration = bool(request.get("allow_cross_calibration", False))
    if not JOBS["send_key"].start(allow_cross_calibration=allow_cross_calibration):
      self._send_json(
        {
          "ok": False,
          "status": "running",
  ...
  def _post_clear_cache(self) -> None:
    can_running = JOBS["can"].is_running()
    df_running = JOBS["dataflash"].is_running()
  ...
  def _handle_request(self, send_body: bool) -> None:
    ...
    if path == "/api/evidence-bundle":
      try:
        ...
        self._send_json({"ok": False, "message": str(e), "traceback": traceback.format_exc()}, status=HTTPStatus.INTERNAL_SERVER_ERROR, send_body=send_body)
      return
    ...
    if path == "/api/xcp-observer-status":
      payload = JOBS["xcp_observer"].snapshot()
      self._send_json(payload, send_body=send_body)
      return

    if path == "/api/ident-map-status":
      payload = JOBS["identity"].snapshot()
      self._send_json(payload, send_body=send_body)
      return

    if path == "/api/reset-probe-status":
      payload = JOBS["reset"].snapshot()
      self._send_json(payload, send_body=send_body)
      return

    if path == "/api/level3-status":
      payload = JOBS["level3"].snapshot()
      self._send_json(payload, send_body=send_body)
      return

    if path == "/api/sendkey-status":
      payload = JOBS["send_key"].snapshot()
      self._send_json(payload, send_body=send_body)
      return

    if path == "/api/preamble-status":
      payload = JOBS["preamble"].snapshot()
      self._send_json(payload, send_body=send_body)
      return

    if path == "/api/uds-sweep-status":
      payload = JOBS["uds_sweep"].snapshot()
      self._send_json(payload, send_body=send_body)
      return

    if path == "/api/ready-capture-status":
      payload = JOBS["ready_passive"].snapshot()
      self._send_json(payload, send_body=send_body)
      return

    if path == "/api/ready-diff-status":
      payload = JOBS["ready_active_diff"].snapshot()
      self._send_json(payload, send_body=send_body)
      return
