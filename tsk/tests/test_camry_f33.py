...
class TestCamryF33Evidence(unittest.TestCase):
  ...
  def test_exact_f33_uses_normal_port_shape(self):
    ...
    self.assertFalse(CAMRY_F33_OPENDBC["supported_output"])
    self.assertTrue(CAMRY_F33_OPENDBC["lateral_request_decoding"])

    self.assertTrue(CAMRY_F33_LATERAL_PORT["available"])
    self.assertEqual(CAMRY_F33_LATERAL_PORT["status"], "native-port-b6-admission-unproven")
    ...
    self.assertIn("admission remains unproven", CAMRY_F33_LATERAL_PORT["receiver_acceptance"])
    self.assertIn("native openpilot longitudinal", CAMRY_F33_LATERAL_PORT["unsupported_features"])
    self.assertNotIn("driver-override", CAMRY_F33_LATERAL_PORT["unsupported_features"])
    self.assertFalse(CAMRY_F33_LATERAL_PORT["supported_output"])

    self.assertEqual(CAMRY_F33_CHECKPOINT["state"], "normal-port-b6-admission-unproven")
    self.assertIn("do not prove", CAMRY_F33_CHECKPOINT["output_detail"])
    self.assertIn("stock-ACC cancel is implemented", CAMRY_F33_CHECKPOINT["unsupported_feature"])
    ...
    self.assertFalse(status["supported_output"])
    self.assertEqual(tuple(status["remaining_research_boundaries"]), CAMRY_F33_REMAINING_RESEARCH_BOUNDARIES)
    self.assertEqual(len(status["remaining_research_boundaries"]), 6)
    self.assertIn("B6 application admission", status["remaining_research_boundaries"][0])
    self.assertIn("stock-ACC cancel is already recovered", status["remaining_research_boundaries"][1])
    self.assertIn("vehicle dynamics", status["remaining_research_boundaries"][4])
    self.assertIn("RAM-only", status["remaining_research_boundaries"][5])
  ...
  def test_lateral_request_is_representation_not_ingress(self):
    ...
    self.assertEqual(req["eps_external_target_ingress"]["address"], 0x0B6)
    self.assertIn("only recovered external target-bearing", req["eps_external_target_ingress"]["basis"])
    self.assertIn("ordinary-SecOC structural match", req["security_trailer"]["classification"])
    self.assertIn("no exact 0x08A-to-B6 transform", req["boundary"])
