...
class TestBootstrapProfile(unittest.TestCase):
  def test_exact_targets_are_evidence_graded(self):
    expected = {
  ...
  def test_public_ram_fixture_is_exactly_evidenced_only_on_4512000(self):
    self.assertTrue(fixture_is_evidenced("8965B4512000", RAM_DUMP_FIXTURE_SHA256))
    self.assertEqual(require_evidenced_fixture("8965B4512000", RAM_DUMP_FIXTURE_SHA256).software_id,
                     "8965B4512000")
    for f181 in ("8965B4209000", "8965B4233100", "8965B4509100", "8965H1202000", "8965F1208000"):
      self.assertFalse(fixture_is_evidenced(f181, RAM_DUMP_FIXTURE_SHA256), f181)
