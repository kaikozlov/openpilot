...
FIELD_GUIDANCE = {
  ...
  "safety_flags": (
    ...
    "The source audit recomputes EPS scale + SecOC/LTA/stock-long/alt-brake bits; for a " +
    "TSS3 platform handled by its own interface branch it recomputes EPS scale + " +
    "STOCK_LONGITUDINAL + TSS3 bits instead."
  ),
  ...
  "longitudinal_control": (
    "Canonical intended mode: openpilot_default, stock_default, or openpilot_alpha. "
    + "This drives the STOCK_LONGITUDINAL safety bit audit; a TSS3 platform handled by its own "
    + "interface branch forces stock_default."
  ),
